...
class Generator(nn.Module):
    ...
    def __init__(self, input_length: int, output_length: int):
        ...
        self.dense_layer_1 = nn.Linear(int(input_length), 256)
        self.dense_layer_2 = nn.Linear(256, 128)
        self.dense_layer_3 = nn.Linear(128, 256)
        self.dense_layer_4 = nn.Linear(256, output_length)
        self.activation = nn.Tanh()
        self.model = nn.Sequential(self.dense_layer_1, self.activation, self.dense_layer_2, self.activation, self.dense_layer_3, self.activation, self.dense_layer_4)
        #self.identity_init()
...
def train(dataset: ClipCocoDataset, model: ClipCaptionModel, args,
    ...
    for epoch in range(epochs):
        ...
        for idx, (tokens, mask, prefix_im, prefix_text) in enumerate(train_dataloader):
            ...
            loss_idt_text = criterionId(idt_text, prefix_text) * lambda_im * lambda_idt
            # G_B should be identity if real_A is fed: ||G_B(A) - A||
            idt_im = generator_im(prefix_im)
            loss_idt_im = criterionId(idt_im, prefix_im) * lambda_text * lambda_idt
            ...
            loss_cycle_text = criterionCycle(rec_text, prefix_text) * lambda_text
            # Backward cycle loss || G_A(G_B(B)) - B||
            loss_cycle_im = criterionCycle(rec_im, prefix_im) * lambda_im
            # combined loss and calculate gradients
            loss_G = loss_G_text + loss_G_im + loss_cycle_text + loss_cycle_im + loss_idt_text + loss_idt_im
            loss_G.backward()
        ...
        if epoch % args.save_every == 0 or epoch == epochs - 1:
            torch.save(
                    generator_im.state_dict(),
                    os.path.join(output_dir, f"generator_im-{epoch:03d}_id15.pt"),
                )
            torch.save(
                    generator_text.state_dict(),
                    os.path.join(output_dir, f"generator_text-{epoch:03d}_id15.pt"),
                )
            torch.save(
                    discriminator_im.state_dict(),
                    os.path.join(output_dir, f"discriminator_im-{epoch:03d}_id15.pt"),
                )
            torch.save(
                    discriminator_text.state_dict(),
                    os.path.join(output_dir, f"discriminator_text-{epoch:03d}_id15.pt"),
                )
