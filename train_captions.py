...
def train(dataset: ClipCocoDataset, model: ClipCaptionModel, args,
    ...
    for epoch in range(epochs):
        ...
        for idx, (tokens, mask, prefix_im, prefix_text) in enumerate(train_dataloader):
            model.zero_grad()
            tokens, mask, prefix = tokens.to(device), mask.to(device), prefix_text.to(device, dtype=torch.float32)
        ...
        if epoch % args.save_every == 0 or epoch == epochs - 1:
            torch.save(
                model.state_dict(),
                os.path.join(output_dir, f"{output_prefix}-{epoch:03d}_text_decoder_onlytrain.pt"),
            )
    return model
