...
def train(dataset: ClipCocoDataset, model: ClipCaptionModel, args,
    ...
    ################LOading modle############
    #model.load_state_dict(torch.load("./coco_train/coco_prefix-004_image.pt"))
    ...
    for epoch in range(epochs):
        ...
        for idx, (tokens, mask, prefix_im, prefix_text) in enumerate(train_dataloader):
            model.zero_grad()
            tokens, mask, prefix = tokens.to(device), mask.to(device), prefix_im.to(device, dtype=torch.float32)
        ...
        if epoch % args.save_every == 0 or epoch == epochs - 1:
            torch.save(
                model.state_dict(),
                os.path.join(output_dir, f"{output_prefix}-{epoch:03d}_image_onlytrain.pt"),
            )
    return model
