data_path = './data/coco/oscar_split_ViT-B_32_images_captions_onlyvalid.pkl'
import pickle
import sys
import torch
...
for i in range(len(prefixes_im)):
    ...
    for j in range(len(prefixes_im)):
        ...
        if cosine_similarity(prefix_text,im_prefix_translated_bis)>cosine_similarity(prefix_text,matching_image_cycle) :
            matching_image_cycle = im_prefix_translated_bis
            c=True
            
        if cosine_similarity(prefix_im,text_prefix_translated_bis)>cosine_similarity(prefix_im,matching_caption_cycle) :
    ...
    if cosine_similarity(matching_image_cycle,im_prefix_translated_bis)==1:
        score_matching_im_cycle +=1
    if cosine_similarity(matching_caption_cycle,text_prefix_translated_bis)==1:
